...
void vectorized::mul_assign(const uint32_t size, double const * __restrict__ iarray, double coeff, double* __restrict__ oarray) {
    for (uint32_t i = 0; i < size; ++i) {
        oarray[i] = coeff * iarray[i];
    } 
}
void vectorized::add(const uint32_t size, double const * __restrict__ iarray, double* __restrict__ oarray) {
    for (uint32_t i = 0; i < size; ++i) {
        oarray[i] += iarray[i];
    } 
}
