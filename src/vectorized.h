...
namespace vectorized {
    ...
    // oarray = coeff * iarray
    void mul_assign(const uint32_t size, double const * __restrict__ iarray, double coeff, double* __restrict__ oarray) ;

    // oarray += iarray
    void add(const uint32_t size, double const * __restrict__ iarray, double* __restrict__ oarray) ;
